...
class posteriorParamsVar(posteriorParamsBase):
    ...
    def _recompute(LLT0, Kmn, ga_approx, covCorr_trace):
        ...
        Sigma_diag = Sigma_diag + covCorr_trace
        return posteriorParamsVar(mu, Sigma_diag), LLT
...
class EP_Var(EPBase, VarDTC):
    def inference(self, kern, X, Z, likelihood, Y, mean_function=None, Y_metadata=None, Lm=None, Knn_diag=None, dL_dKmm=None,
        ...
        if self.ep_mode == "nested":
            # Force EP at each step of the optimization
            self._ep_approximation = None
            post_params, ga_approx, log_Z_tilde = self._ep_approximation = self.expectation_propagation(Knn_diag, Kmm, Kmn,
                                                                                                        Y, likelihood,
                                                                                                        Y_metadata)
        elif self.ep_mode == "alternated":
            if getattr(self, '_ep_approximation', None) is None:
                # if we don't yet have the results of runnign EP, run EP and store the computed factors in self._ep_approximation
                post_params, ga_approx, log_Z_tilde = self._ep_approximation = self.expectation_propagation(Knn_diag, Kmm,
                                                                                                            Kmn, Y,
                                                                                                            likelihood,
                                                                                                            Y_metadata)
            else:
        ...
        return super(EP_Var, self).inference(kern, X, Z, likelihood, ObsAr(mu_tilde[:, None]),
                                            mean_function=mean_function,
                                            Y_metadata=Y_metadata,
                                            precision=ga_approx.tau,
                                            Lm=Lm, dL_dKmm=dL_dKmm,
    ...
    def expectation_propagation(self, Knn_diag, Kmm, Kmn, Y, likelihood, Y_metadata):
        num_data, output_dim = Y.shape
        assert output_dim == 1, "This EP methods only works for 1D outputs"
        ...
        marg_moments = marginalMoments(num_data)
        cav_params = cavityParams(num_data)
        ga_approx, post_params, LLT0, LLT, covCorr_diag = self._init_approximations(Knn_diag, Kmm, Kmn, num_data)

        stop = False
        iterations = 0
        while not stop and (iterations < self.max_iters):
            self._local_updates(num_data, LLT0, LLT, Kmn, cav_params, post_params, marg_moments, ga_approx, likelihood, Y, Y_metadata, covCorr_diag)
            #(re) compute Sigma, Sigma_diag and mu using full Cholesky decompy
            post_params, LLT = posteriorParamsVar._recompute(LLT0, Kmn, ga_approx, covCorr_diag)
            post_params.Sigma_diag = np.maximum(post_params.Sigma_diag, np.finfo(float).eps)
    ...
    def _init_approximations(self, Knn_diag, Kmm, Kmn, num_data):
        #initial values - Gaussian factors
        #Initial values - Posterior distribution parameters: q(f|X,Y) = N(f|mu,Sigma)
        LLT0 = Kmm.copy()
        Lm = jitchol(LLT0) #K_m = L_m L_m^\top
        ...
        covCorr_diag = Knn_diag - Qnn_diag
